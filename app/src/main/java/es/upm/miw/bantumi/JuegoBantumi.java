...
public class JuegoBantumi {
    ...
    public static final int USER_POSITION = 6;
    public static final int RIVAL_POSITION = 13;
    ...
    public Integer userScore(){
        return this.getSemillas(USER_POSITION);
    }

    public Integer opponentScore(){
        return this.getSemillas(RIVAL_POSITION);
    }
    ...
    public String serializa() {
        // @TODO
        String serializada="";
        serializada = serializada.concat(this.getDate().toString() + ">");
        serializada = serializada.concat((this.bantumiVM.getTurno().getValue()).toString() + ">");
        serializada = serializada.concat(this.getMatchScore() + ">");
    ...
    public void deserializa(String juegoSerializado) {
        ...
        String[] valores = juegoSerializado.split(">");

        this.setTurno(Turno.valueOf(valores[1]));

        String[] arrayString=valores[2].split("-");
        for (int i = 0; i < NUM_POSICIONES; i++) {
            this.setSemillas(i, Integer.parseInt(arrayString[i]));
        }
    }

    protected String getMatchScore(){
        String tableSemillas = new String();
        for (int i = 0; i < NUM_POSICIONES; i++) {
            tableSemillas += this.getSemillas(i) + "-";
        }
        return tableSemillas;
    }
}
